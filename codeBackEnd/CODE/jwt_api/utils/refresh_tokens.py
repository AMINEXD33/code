...
class Refresh_tokens_manager:
    ...
    def create_new_refresh_token():
        ...
        exp_date = datetime.datetime.now() + timedelta(
            days=Refresh_tokens_manager.EXPERATION_RANGE
        )
    ...
    def cache_refresh_token(
        redis_conn_instance, token, exp_date, devices_array, username, id_, role
    ):
        """
    ...
    def check_cached_refresh_token(Redis_conn, token: str, token_isonformate_expdate):
        ...
        if type(loaded_ison_format) == type(""):
            loaded_ison_format = datetime.datetime.fromisoformat(
                token_isonformate_expdate
            )
